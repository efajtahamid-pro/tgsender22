...
def accounts():
    ...
    # Check if there is any proxy with less than 5 accounts
...
def batch_add_accounts():
    ...
    for phone in phones:
        ...
        if not assigned_proxy:
            proxy_failed += 1
            break # Stop processing if proxies are full
        db.session.add(account)
        added += 1
...
def employees():
    ...
    if request.method == 'POST':
        ...
        # Normalize blank email to None -- an empty string is not NULL, so a
        # second employee left without an email would collide on the unique
        # email constraint and raise an uncaught IntegrityError.
        email = (request.form.get('email') or '').strip() or None
        # Simplified: Only handle replies permission
        ...
        try:
            ...
            db.session.rollback()
            flash('Could not create employee: username or email already in use.', 'danger')
        return redirect(url_for('admin.employees'))
