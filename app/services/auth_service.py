...
def validate_strong_password(password: str):
    if not password or len(password) < 10: return False, 'Password must be at least 10 characters long.'
...
def register_failed_login(user: User):
    ...
    if user.failed_login_attempts >= current_app.config.get('LOGIN_MAX_ATTEMPTS', 5):
...
def is_locked(user: User) -> bool:
    if not user.locked_until: return False
    if datetime.utcnow() < user.locked_until: return True
    user.locked_until = None
    user.failed_login_attempts = 0
    db.session.commit()
    return False
