...
def register_socket_handlers(socketio):
    ...
    def on_disconnect():
        if current_user.is_authenticated: logger.info('Socket disconnected', extra={'user_id': current_user.id})
    ...
    def on_join_campaign(data):
        ...
        if not campaign: return disconnect()
    ...
    def on_join_conversations():
        if current_user.is_authenticated: join_room(f'user_{current_user.id}')
