...
def update_heartbeat(status, error=None):
    try:
        db.session.execute(text("""
            ...
            ON CONFLICT (worker_name) DO UPDATE SET last_heartbeat = NOW(), status = :status, last_error = :error
...
def reset_stuck_recipients():
    ...
    for r in stuck: r.status = 'pending'
    db.session.commit()
...
def reset_daily_limits():
    ...
    for camp in db.session.query(Campaign).filter(Campaign.status.in_(['paused', 'running'])).all():
        if camp.last_reset and (now - camp.last_reset) >= timedelta(hours=24):
            camp.daily_sent = 0; camp.last_reset = now
            if camp.status == 'paused' and camp.pause_reason == 'daily_limit_reached': camp.status = 'running'; camp.pause_reason = None
    db.session.commit()

def claim_recipient(campaign_id):
    result = db.session.execute(text("""
        UPDATE recipients SET status = 'sending'
        WHERE id = (SELECT id FROM recipients WHERE campaign_id = :cid AND status = 'pending' ORDER BY id FOR UPDATE SKIP LOCKED LIMIT 1)
...
def process_campaign(campaign, telegram):
    if campaign.daily_sent >= campaign.daily_limit:
        campaign.status = 'paused'; campaign.pause_reason = 'daily_limit_reached'; db.session.commit(); return
    ...
    if not claimed:
        if not db.session.query(Recipient).filter_by(campaign_id=campaign.id, status='pending').first():
            campaign.status = 'completed'; campaign.completed_at = datetime.utcnow(); db.session.commit()
        return
    ...
    account = db.session.query(TelegramAccount).filter(
        ...
        TelegramAccount.is_active == True, TelegramAccount.is_verified == True,
        TelegramAccount.daily_sent < TelegramAccount.daily_limit
    ).order_by(TelegramAccount.last_used.asc().nullsfirst()).first()

    if not account:
        recipient.status = 'pending'; db.session.commit()
        campaign.status = 'paused'; campaign.pause_reason = 'no_available_accounts'; db.session.commit(); return
    ...
    result = telegram.send_message_sync(account, username.lstrip('@'), chosen_message)
    log_entry.completed_at = datetime.utcnow()

    if result['status'] == 'success':
        recipient.status = 'sent'; recipient.sent_at = datetime.utcnow(); recipient.assigned_account_id = account.id
        recipient.user_id = result.get('telegram_user_id')
        account.daily_sent += 1; campaign.daily_sent += 1; account.last_used = datetime.utcnow()
        log_entry.status = 'success'; log_entry.telegram_message_id = result.get('telegram_message_id')
        if not recipient.conversation: db.session.add(Conversation(recipient_id=recipient.id, campaign_id=campaign.id, employee_id=campaign.employee_id))
    else:
        recipient.retry_count += 1; recipient.last_error = result.get('message')
        log_entry.status = 'failed'; log_entry.error = recipient.last_error
        if result.get('permanent') or recipient.retry_count >= 3:
            recipient.status = 'dead_letter'; recipient.dead_letter_reason = recipient.last_error
        else: recipient.status = 'pending'
...
def run():
    ...
    with app.app_context():
        ...
        reset_stuck_recipients()
        logger.info("Campaign Worker started.")
        while True:
            try:
                update_heartbeat('running'); reset_daily_limits()
                for camp in db.session.query(Campaign).filter_by(status='running').all():
                    process_campaign(camp, telegram)
                time.sleep(2)
            except Exception as e:
                logger.exception("Campaign worker crashed")
                update_heartbeat('error', str(e)); time.sleep(10)
